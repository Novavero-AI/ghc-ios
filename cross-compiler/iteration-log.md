...
Nobody has documented a working GHC 9.8 iOS cross-compiler build from scratch.
This log records every failure and fix across ~30 iterations in a single day.
...
### Phase 6 — Hadrian Build System (v32+)
...
### Phase 8 — Host GHC Quirks (post-bootstrap)

Discovered during a downstream `cabal build` of `truthcapture-app`. Every host link on macOS emits:
...
1. Cache check (keyed `ghc-9.8.4-aarch64-apple-ios-v37`)
...
10. Install: same Hadrian flags as build (Hadrian doesn't persist CLI settings)
...
### Phase 9 - Public repo bootstrap (v38)
